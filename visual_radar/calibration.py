...
def _maps_exist(folder: Path) -> bool:
...
def load_calibration(calib_dir: Path,
    ...
    if calib_dir and _maps_exist(calib_dir):
        ...
        Q     = np.load(calib_dir / "Q.npy", allow_pickle=True)
        size_ref = (map1x.shape[1], map1x.shape[0])
        print("[*] Calibration: metric (maps from disk).")
        return Calibration(mode="metric_maps", map1x=map1x, map1y=map1y,
                           map2x=map2x, map2y=map2y, Q=Q, size_ref=size_ref)

    if intrinsics_npz and Path(intrinsics_npz).exists():
        data = np.load(intrinsics_npz, allow_pickle=True)

        K1 = _pick(data, "K1", "cameraMatrix1")
        K2 = _pick(data, "K2", "cameraMatrix2")
        D1 = _pick(data, "D1", "distCoeffs1")
        D2 = _pick(data, "D2", "distCoeffs2")
        R  = _pick(data, "R")
        T  = _pick(data, "T")
        ...
        size = (w, h)
        R1, R2, P1, P2, Q, roi1, roi2 = cv.stereoRectify(K1, D1, K2, D2, size, R, T,
        ...
        return Calibration(mode="metric_compute", map1x=map1x, map1y=map1y,
                           map2x=map2x, map2y=map2y, Q=Q, size_ref=size)
...
def rectified_pair(calib: Calibration, imgL, imgR):
    if calib.mode.startswith("metric"):
        ...
        if (w, h) != (mw, mh):
            ...
            # ВНИМАНИЕ: Для точной 3D-метрики лучше пересчитать калибровку под этот размер.
