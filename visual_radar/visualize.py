...
def draw_boxes(img, boxes: Iterable[BBox], color=(0, 255, 0), tag: str = "", thickness: int = 2) -> None:
    """Аккуратные боксы с подписью."""
    if img is None:
        return
    for b in boxes:
        x, y, w, h = b.to_int()
...
def stack_lr(left_bgr, right_bgr):
    """Горизонтальная склейка с выравниванием по высоте (без искажений)."""
...
def imshow_resized(win: str, img, max_w: int = 1920, max_h: int = 1080) -> None:
    """Показываем в окно, подгоняя под экран (WINDOW_NORMAL)."""
    if img is None:
        return
    h, w = img.shape[:2]
...
def draw_hud(img, lines: List[str], corner: str = "tr", alpha: float = 0.55) -> None:
    """
    Полупрозрачный HUD с текстом.
    corner: 'tl' | 'tr' | 'bl' | 'br'
    """
